...
namespace NoteNough.NET
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                var envAudience = Environment.GetEnvironmentVariable("DOMAIN_URL") ?? "localhost:3000";
            ...
            builder.Services.AddCors(options =>
            {
                options.AddPolicy(name: myAllowSpecificOrigins,
                                  policy =>
                                  {
                                      policy.WithOrigins(jwtConfig.Issuer, jwtConfig.Audience, "http://localhost:3000")
                                      .AllowAnyMethod()
                                      .AllowAnyHeader()
                                      .AllowCredentials();
                                  });
